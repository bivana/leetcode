...
public class P1619MeanOfArrayAfterRemovingSomeElements{
    ...
    @Test
    public void test(){
        Assert.assertEquals(2.0,solution.trimMean(new int[]{1,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,3}),0.01);
    }
    ...
    class Solution {
        public double trimMean(int[] arr) {
            Arrays.sort(arr);
            int removeCnt=(int)(arr.length*0.05);
            int remainCnt=arr.length-2*removeCnt;
            double sum=0.0;
            for(int i=removeCnt;i<arr.length-removeCnt;i++){
                sum+=arr[i];
            }
