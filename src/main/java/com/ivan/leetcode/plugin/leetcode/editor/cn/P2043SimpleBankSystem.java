...
// 
// Related Topics 设计 数组 哈希表 模拟 👍 32 👎 0
...
class Bank {
    ...
    public boolean transfer(int account1, int account2, long money) {
        if((!isValidAccount(account1,money))||(!isValidAccount(account2))){
            return false;
        }
    ...
    private boolean isValidAccount(int account,long money){
    ...
    public boolean withdraw(int account, long money) {
        if(!isValidAccount(account,money)){
            return false;
        }
        balance[account-1]-=money;
        return true;
