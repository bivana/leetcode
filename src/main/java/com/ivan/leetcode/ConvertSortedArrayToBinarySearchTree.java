...
public class ConvertSortedArrayToBinarySearchTree {
    ...
    public TreeNode sortedArrayToBST(int[] nums,int start,int end,TreeNode treeNode) {
        if(start>end){
            return null;
        }
        ...
        if(treeNode!=null){
            if(child.val<=treeNode.val){
                treeNode.left=child;
            }else{
                treeNode.right=child;
            }
        }
        sortedArrayToBST(nums,start,mid-1,child);
        sortedArrayToBST(nums,mid+1,end,child);
        return child;
    }
}
